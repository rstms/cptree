...
def test_cp_local_local(local_src, local_dst):
    ret = cptree(local_src, local_dst, create=True, delete=True)
    assert ret == 0


def test_cp_local_remote(local_src, remote_target):
    ret = cptree(local_src, remote_target, create=True, delete=True)
    assert ret == 0


def test_cp_remote_local(remote_target, local_dst):
    ret = cptree(remote_target, local_dst, create=True, delete=True)
    assert ret == 0
...
def test_cp_badlocal_local(local_dst):
    with pytest.raises(InvalidDirectory):
        cptree("bad_local_src", local_dst, create=True, delete=True)


def test_cp_badremote_local(remote_host, local_dst):
    bad_remote_src = f"{remote_host}:bad_remote_src"
    with pytest.raises(InvalidDirectory):
        cptree(bad_remote_src, local_dst, create=True, delete=True)
...
def test_cp_progress(big_local_src, remote_target):
    cptree(big_local_src, remote_target, create="ask", delete="ask")
